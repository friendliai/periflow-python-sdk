...
def main(args):
    ...
    if args.resume:
        if len(ckptdirlist := os.listdir(args.resume)) == 0:
            raise RuntimeError("Directory specified by `--resume` has no checkpoint files.")
        ckptfile = ckptdirlist[0]
        checkpoint = torch.load(os.path.join(args.resume, ckptfile), map_location="cpu")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir:
            ...
            # Actually saves only in the main process
