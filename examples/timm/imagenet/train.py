...
def main():
    ...
    if args.initial_checkpoint:
        _logger.info("Initial checkpoint directory specified: " + args.initial_checkpoint)
        # search for checkpoint file
        ckptdir = os.listdir(args.initial_checkpoint)[0]
        args.initial_checkpoint = os.path.join(args.initial_checkpoint, ckptdir, "last.pth.tar")
        if not os.path.exists(args.initial_checkpoint):
            raise RuntimeError("Directory specified by `--initial-checkpoint` does not include checkpoint files.")
    ...
    if args.model_ema:
        ...
        if args.resume:
            load_checkpoint(model_ema.module, args.initial_checkpoint, use_ema=True)
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            if output_dir is not None:
                try:
                    ...
                    print("#"*30, "FILE NOT FOUND ERROR")
                    print("output_dir =", output_dir)
                    os.system("ls /workspace/ckpt")
                    if os.path.exists(output_dir):
                        print("OUTPUT_DIR EXISTS")
                        os.system("ls " + str(output_dir))
                    else:
